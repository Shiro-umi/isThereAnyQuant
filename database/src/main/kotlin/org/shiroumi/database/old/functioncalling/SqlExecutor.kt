package org.shiroumi.database.old.functioncalling
...
import org.shiroumi.database.old.table.*
...
fun fetchDoneTasks(): List<List<String>> = stockDb.transaction {
    listOf()
//    Strategy.Companion.all().sortedByDescending { it.startTime }.map { s ->
//        listOf("${s.id}", s.tsCode, s.name, s.tradeDate, s.startTime.toString())
//    }
}

fun fetchDoneTask(uuid: String): String = stockDb.transaction {
//    Strategy.Companion.findById(UUID.fromString(uuid))?.strategy ?: ""
    ""
}

fun getStockName(tsCode: String): String = stockDb.transaction {
//    val res = Stock.Companion.find { StockTable.tsCode eq tsCode }.toList().firstOrNull()
...
//    val symbolized = Symbolized.Companion.new {

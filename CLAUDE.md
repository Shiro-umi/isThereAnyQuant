...
## Post-Commit Branch Sync — 提交后统一并入 main（项目级最高规范）
本规范是项目级最高规范，优先级高于其他流程约定。代码提交完成后执行本规范。

### 前置条件：工作空间干净
- 干净判定只看业务代码是否已全部提交。
- 干净判定忽略 deploy 产物类变更，包括 `compose-app/version.properties`、`iosApp/Configuration/Config.xcconfig` 等部署自动 bump 的版本号文件。
- 业务代码全部提交即视为工作空间干净，部署版本号文件的残留变更不阻断本规范。

### 动作：所有 git 项目并入 main
- 把所有 git 项目同步至 `main` 分支，范围覆盖主工程与所有 submodule。
- 本规范是合并语义。功能分支（`feat/*`）的提交并入各自的 `main`，各仓最终停在 `main`。

### 顺序铁律：先 submodule 后主工程
- 先把每个 submodule 并入并停在其 `main`。
- 再把主工程并入并停在其 `main`。
- 主工程提交记录的 private 指针指向 private submodule 的 `main`。

### 期望终态
- 项目完全干净。
- 主工程在 `main` 分支。
- private submodule 在 `main` 分支。
- 主工程记录的 private 指针指向 private 的 `main`。

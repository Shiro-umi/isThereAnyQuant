...
interface Logger {
    ...
    fun startAccept() = print("$GREEN[ACCEPT] [$className] ")

    fun startNotify() = print("$PURPLE[NOTIFY] [$className] ")

    fun append(content: String) = print(content)

    fun end() = println(RESET)

    fun w(content: String) = print("$YELLOW$content$RESET")
}
